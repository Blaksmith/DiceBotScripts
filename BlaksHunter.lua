...
-- minChance = 2.50
chanceInc     = 0.00001 -- How much to increase chance on a bad run
...
if(appendlog != true) then
    ...
    fin:write("          Timestamp,     Bet ID,  Strk,        Bet, Chance,      Spent, Win Amount, Win Profit,   Roll,   High/Low\n")
    fin:close()
end
        ...
        if(win == true) then -- reset? 
            ...
            if(targetAverage >= stopHunt and isHunting == true and abortHuntFlag == true) then -- reset to seeking
                ...
                if(huntAmount > abortHunt and abortHuntFlag == true) then
                    ...
                    chance = chance + chanceInc -- May tweak this later.
                end
            end
        end
    end
...
function dobet()
    ...
    if win then -- Process a win
        ...
        if(lastBet.Roll >= 99.9999 and isHunting == true) then logWin() end
